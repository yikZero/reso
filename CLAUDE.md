...
- `koe-core/` — Rust: config, ASR session management, LLM correction, FFI exports (`sp_core_*`)
- `koe-asr/` — Rust: Doubao/Volcengine WebSocket streaming ASR client
...
- User config lives at `~/.koe/config.yaml` (YAML, hot-reloaded every 3s)
- Default prompts embedded in `koe-core/src/default_system_prompt.txt` and `default_user_prompt.txt`
